...
\textbf{WARNING 2: Since we rewrote the library to use placeholders, the library is significantly slower (7 times slower!). I need to understand why, and see how to fix it. Hopefully I can fix it without important change, but it might introduce backward-incompatible changes in the future. So do not consider this version as stable, and definitely copy the .sty file if you plan to use it.}
...
We benchmarked the library before a major rewrite introducing placeholders (benchmark was in commit \href{https://github.com/leo-colisson/robust-externalize/commit/09a05a86ec9c04eb6687b75831ebb84185786ac5}{09a05a86ec9c04eb6687b75831ebb84185786ac5}), and the library was performing really well. On an early draft of a small paper containing 76 small tikz-cd based pictures (from my other zx-calculus library), we measured on a first version of this library:
\begin{itemize}
...
BUT on the latest version 1.0+unstable, the benchmark it actually not as good. Tested on a different file with 318 pictures:
\begin{itemize}
\item without externalization: 1mn36
\item with externalization (first run): 3mn25, more or less like in the original case
\item with externalization (next runs): 20 seconds (hum… not really good, it is only 5x times faster)
\item with |enable optimizations| enabled (next runs): 12 seconds (better, 8x faster), but can certainly do better
\item with old version (next runs): 3.74 seconds (hum… why is the old version so much faster, it is 25x faster here!)
\end{itemize}
After some experimentation, it seems like \LaTeX{} is really slow with string manipulations. The old version was doing a single rewrite of the string, while we do much more rewrite due to the placeholder system: it is much more flexible, but

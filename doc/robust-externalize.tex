...
\begin{pgfmanualentry}
  \extractcommand\robExtWrapEnvironment\marg{name environment}\marg{default preset options}\@@
  \pgfmanualbody
  ...
  \begin{codeexample}[width=0pt,vbox]
    \robExtWrapEnvironment{ZX}{latex, add to preamble={\usepackage{zx-calculus}}}
    A ZX figure %
    \begin{ZX}
...
Note that we provide no argument to wrap automatically an arbitrary command since it is hard to know which part of the command should be kept in the file, and which should not: for instance, in |\foo{42}|, |{42}| might either be a group unrelated to |\foo|, or an argument to |\foo|. However, you can define yourself your own wrapping (even if it might be easier to simply create a new function instead of overwriting an existing macro). Note that if you want |disable externalization| to work on this command, you should define |\robExtCommandOrigYOURMACRONAME| that should point to the original definition of the macro.
...
\item v1.1:
  ...
  \item you can now automatically cache environments and commands using |\robExtWrapEnvironment|
  \end{itemize}
\end{itemize}

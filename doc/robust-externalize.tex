...
\textbf{WARNING: auto-forwarding (and more generally all options that need to read the document to add new style) can add a noticeable time to the compilation since regex search in \LaTeX{} is quite slow. For instance, on a benchmark with a compiled preset, without any caching the document takes 1mn54 to compile, with the compiled preset it takes 4.5s to compile (25x faster), and with auto forward it takes 7.8s to compile. Any new |if matches regex| also adds 2s to the whole compilation time in this benchmark, but |if matches| and |forward color| seems to add a negligible time.}
...
\end{tikzpicture}  
\end{codeexample}
\textbf{WARNING: auto-forwarding can add a noticeable time to the compilation since regex search in \LaTeX{} is quite slow. For instance, on a benchmark (v2.0+2023-11-09) with a compiled preset, without any caching the document takes 1mn54 to compile, with the compiled preset it takes 4.5s to compile (25x faster), and with auto forward it takes 7.8s to compile. Any new |if matches regex| also adds 2s to the whole compilation time in this benchmark, but |if matches| and |forward color| seems to add a negligible time.}
\end{pgfmanualentry}

...
\begin{center}
  ...
  \vspace{1em}
  {Léo Colisson \quad Version 2.0+unstable}\\[3mm]
...
\item We show how to compile all images in parallel using |\robExtConfigure{compile in parallel}| (make sure to have GNU |parallel| installed on Linux/Mac and rush on Windows if you use the default command) even during the first run, so that the first run becomes faster to compile than a normal run(!)
...
The first run might be particularly long to compile since many elements are still not cached and re-starting \LaTeX{} on each picture can take quite some time. If you have v2.1 or above, and if you have installed GNU parallel (on linux), or rush (on Windows) (\url{https://github.com/shenwei356/rush/}), you can simply compile all images in parallel by typing at the beginning of the file:
\begin{codeexample}[code only]
\robExtConfigure{
  compile in parallel
}
...
\newcommandAutoForward{\MyNode}[2][draw,thick]{\node[rounded corners,fill=red,#1]{\ding{164} \MyName: #2};}[forward=\MyName, add to preamble={\usepackage{pifont}}]
...
Make sure to have GNU parallel installed (linux/mac) or rush (windows), and type in your preamble:
\begin{codeexample}[code only]
\robExtConfigure{
  compile in parallel
}
\end{codeexample}
to compile all figures in parallel (you need to compile your document twice). Your document should build significantly faster, even faster than a normal run. If you want to compile in parallel
only if you have more than 5 new elements to cache, do:
\begin{codeexample}[code only]
\robExtConfigure{
...
\begin{pgfmanualentry}
  ...
  \makeatother%
  \pgfmanualbody
  Enable automatic forwarding of macros marked with |*AutoForward| commands or |\configIfMacroPresent|. This command naively extracts all used macro via a simple regexp match and forwards them/execute the style configured in |\configIfMacroPresent| if it exists.
\begin{codeexample}[vbox]
\cacheTikz  
\robExtConfigure{add to preset={tikz}{auto forward}}
...
\begin{pgfmanualentry}
  \makeatletter%
  \def\extrakeytext{style, }
  \extractkey/robExt/if matches=\marg{latex3 regexp}\marg{style to apply}\@nil
  \makeatother%
  \pgfmanualbody
  Apply the corresponding style if the regexp (in \LaTeX{}3 format) is matched.
\begin{codeexample}[vbox]
\robExtConfigure{
  add to preset={my python}{
    python print code and result,
    % \b is for word border, and ( needs to be escaped in regexp
    if matches={\b cos\(}{add import={from math import cos}},
    if matches={\b sin\(}{add import={from math import sin}},
  },
}
...
\begin{pgfmanualentry}
  \makeatletter%
  \def\extrakeytext{style, }
  \extractkey/robExt/compile in parallel=\oarg{nb of pictures to compile normally} (default 0)\@nil
  \extractkey/robExt/compile in parallel after=\oarg{nb pictures to compile normally} (default 0)\@nil
  \makeatother%
  \pgfmanualbody
...
\begin{pgfmanualentry}
  \makeatletter%
  \def\extrakeytext{style, }
  \extractkey/robExt/compile in parallel command=\marg{command to run} (default 0)\@nil
  \makeatother%
  \pgfmanualbody
  You can customize the compilation command to run via something like:
\begin{codeexample}[code only]
\robExtConfigure{
  compile in parallel command={parallel --jobs 200\% :::: '\jobname-\robExtAddPrefixName{compile-missing-figures.sh}},
}
\end{codeexample}
\end{pgfmanualentry}
...
\end{verbatim}
(note that since v2.1, it is not necessary to remove duplicates using |sort| and |uniq| since the file contains no more duplicates) In my benchmark, it ran during 52s, so 1.6x faster than the original compilation)
...
\item Since externalizing is all about speed, it would be nice to do more benchmarks, in particular to see if automatically forwarding macros slows down the process in a noticeable way.
\end{itemize}

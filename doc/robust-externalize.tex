...
\begin{pgfmanualentry}
  ...
  \makeatother
  If you want to print the whole file in the small error message because you do not want to inspect the full log file, you can use the style |print whole file in error message|. This is enabled by default on the gnuplot preset for instance, but disabled in \LaTeX{} has it makes little sense to print thousands of log lines.
\end{pgfmanualentry}
...
\begin{pgfmanualentry}
  ...
  \makeatother
  Some IDE like TeXstudio will only print lines starting with an exclamation mark, so to make the compilation errors more visible in the log, we can start every line in the log with a |!| (you can set it globally via |logs should show as errors|). This way, all presets that enable |print whole file in error message| will print after the error their log files like:
\begin{verbatim}
-------- We print now the full log --------
! sh: ligne 1: gnuplot : commande introuvable
-------- End of the full log --------
\end{verbatim}
end the like |sh: ligne 1: gnuplot : commande introuvable| will appear in your IDE without going in the log file.
\end{pgfmanualentry}

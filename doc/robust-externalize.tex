...
\textbf{TODO: understand why} BUT on the latest version 1.0+unstable, the benchmark it actually not as good. Tested on 318 pictures:
\begin{itemize}
\item without externalization: 1mn36
\item with externalization (first run): 3mn25, more or less like in the original case
\item with externalization (next runs): 26 seconds (hum… not really good!)
\item with old version (next runs): 3.74 seconds (hum… why is the old version so much faster?)
\end{itemize}
so on the latest version we only get a x3 improvement over a normal compilation while the old version gets a x25 improvement speed\dots{} Seems like \LaTeX{} is really slow with string manipulations, I need to investigate further. So you might still not consider this version as stable yet.

...
\begin{pgfmanualentry}
  ...
  \makeatother
  If you want to print the whole file at that position because you do not want to inspect the full log, you might want to use the style |print whole file in error message|. This is enabled by default on the gnuplot preset for instance.
\end{pgfmanualentry}
...
\begin{pgfmanualentry}
  ...
  \makeatletter%
  \def\extrakeytext{style, }
  \extractkey/robExt/prefix error message with=\marg{string to prepend to error message}\@nil
  \makeatother
  Some IDE like TeXstudio will only print lines containing ``error'', so to make the compilation errors more visible in the log, you can use |prefix error message with=Sub-Error| to generate a message like:
\begin{verbatim}
...
(robExt)                vvvvvv
(robExt)                Sub-Error! Package tikz Error: Giving up on this
(robExt)                path. Did you forget a semicolon?.
(robExt)                
(robExt)                ^^^^^^
...
\end{verbatim}
(note that the indentation might make that the line will be cut in two parts)
\end{pgfmanualentry}

Then, you can look at the log file to get more advices, and run yourself the compilation command printed in the above error message. This way, you can easily check the content of the file and see why it fails to compile. In version before 2.3, or if you use |do not redirect compilation output|, the compilation errors are also displayed directly in the output, and you can read the log file for details.

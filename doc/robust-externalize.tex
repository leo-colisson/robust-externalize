...
You can also disable externalization on elements that contain a specific string/regexp. For instance, you can disable externalization on all elements containing |remember picture|:
\begin{codeexample}[vbox]
\cacheTikz
\robExtConfigure{
  add to preset={tikz}{
    % \b means "word boundary", and spaces must be escaped
    if matches={\b remember\ picture\b}{disable externalization},
  },
}
\begin{tikzpicture}[remember picture]
...
\robExtConfigure{
  add to preset={tikz}{
    ...
    if matches={\b mypink\b}{forward color=mypink},
  },
}
\begin{tikzpictureC}
...
\begin{pgfmanualentry}
  ...
  \makeatother%
  \pgfmanualbody
  Apply the corresponding style if the regexp (in \LaTeX{}3 format, so spaces must be escaped like |\ |) is matched.
\begin{codeexample}[vbox]
\robExtConfigure{

...
\begin{pgfmanualentry}
  ...
  \makeatother
  Some IDE like TeXstudio will only print lines starting with an exclamation mark, so to make the compilation errors more visible in the log, we can start every line in the log with a |!| (you can set it globally via |logs should show as errors|). This way, all presets that enable |texstudio|, or |print whole file in error message| will print after the error their log files like:
\begin{verbatim}
-------- We print now the full log --------
! sh: ligne 1: gnuplot : commande introuvable
-------- End of the full log --------
\end{verbatim}
this way the line |sh: ligne 1: gnuplot : commande introuvable| will appear in your IDE without going in the log file.
\end{pgfmanualentry}

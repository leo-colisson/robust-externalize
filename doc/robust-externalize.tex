...
\textbf{WARNING: This library is young and might lack some testing (and is an important rewrite of a previous version), but v1.1 is definitely usable. Even if we try to stay backward compatible, the only guaranteed way to be immune to changes is to copy/paste the library in your main project folder. Please report any bug to \url{https://github.com/leo-colisson/robust-externalize}!}
...
This library allows you to cache any language: not only \LaTeX{} documents and \tikzname{} images, taking into account depth and overlays, but also arbitrary code. To use it, make sure to have v1.1 installed (see instructions below), and load:
\begin{verbatim}
...
\item with externalization, v1.1 (first run, not in parallel): 3mn05, so takes twice the time; but (first run in parallel) takes 60s (so faster by a factor of 1.5).
...
\subsubsection{Compile a preset}\label{sec:compilePreset}
...
\begin{pgfmanualentry}
  ...
  \pgfmanualbody
  (new v1.1) Create a new macro that executes the \LaTeX{} code in the placeholder.
\end{pgfmanualentry}
...
\subsubsection{The import system}\label{sec:importSystem}
...
\begin{pgfmanualentry}
  ...
  \makeatother%
  \pgfmanualbody
  Print (or shows in the console for the alternative versions) all registered groups of placeholders, mostly for debugging purpose, with the star prints only the content of the inner placeholders.
\begin{codeexample}[width=0pt,vbox]
...
\begin{pgfmanualentry}
  ...
  \extractkey/robExt/add placeholders to group=\marg{name group}\marg{name placeholder}\@nil
  \makeatother%
  \pgfmanualbody
  Create a new group and add placeholders to it (you can put multiple placeholders separated by commas).
\begin{codeAndResult}
\robExtConfigure{
  new group placeholders={my dumy group},
  set placeholder no import={__name__}{Alice},
